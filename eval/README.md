...
| 2026-09-06 | Opus (`run_eval.py --fixture cause --repeat 2`, **v0.81.0 prompt** = v0.74.0 + the F-58 restore-direction clause, the `ACCEPTED` bullet, the harness duration rule, the install-flag rule) | rates (root cause) | **5/7 · 6/7** on the pre-existing rows; the added restore row **0/2**, removed | The payment for the prompt release. Both runs found the truncation, its class and the decoy, isolated the scratch (bytecode row 2/2) and ran clean controls; the misses are the two prose-vocabulary rows ("mechanism" in one run, "trigger" in both) — VERDICT-F-74's class, present in the 2026-09-03 series too. The restore row could not be earned here (see Answer-key amendments): the clause ships measured as harmless, not as an improvement. Scorer note: both workdirs re-score as 0/8 because the fixture's own test command leaves `__pycache__/` untracked — scorer FP #3 again; the per-row points are the measurement. |
| 2026-09-06 | Opus (`run_eval.py --fixture pricer --mode seeded`, v0.81.0 prompt) | pricer delta (the flagship) | **6/6** | Regression check for the four 0.81.0 clauses: all four delta classes, REGRESSED first, quarantine release intact. |
| 2026-09-06 | Opus (`run_eval.py --fixture liar`, v0.81.0 prompt) | liar (adversarial honesty) | **6/6** | Regression check: the honesty rows hold with the new bullets in the prompt. n=1. |
...
### Runs on strangers' repositories

Unscored, because there is no answer key: the published wheel is pointed at a repository
nobody here has opened, in solo mode, the way someone who just installed it would
(`uvx --from verdict-qa-mcp verdict-run <path> --model opus`). The point is not the
findings against the target — those are the maintainers' to weigh — but what the tool
itself does on a codebase and toolchain it was not built beside. Each row records both.

| Date | Wheel | Target | Verdict | What it found there | What it found in Verdict |
|---|---|---|---|---|---|
| 2026-09-05 | 0.80.0 | [pallets/itsdangerous](https://github.com/pallets/itsdangerous) (Python, pytest, 297 tests) | `pass with risks` · 9 findings, all `proven` · 18 min | Signature verification accepts non-canonical base64 (four token strings unsign to one payload); the `max_age` boundary is unpinned (`>`→`>=` leaves 297/297 green); `hmac.compare_digest` can become `==` with nothing failing; unbounded `zlib.decompress` on unverified input via `loads_unsafe`, held to Minor because `loads()` verifies first (measured: 0 decompress calls on a bad signature). Verified intact: verification precedes decompression, key rotation revokes, salts separate contexts, wire-format golden masters. | `verdict-run .` read `.` as a solo *key*, declared the completed run lost, ran it again and exited 4 over two valid runs. Fixed in 0.80.1 the same evening. |
| 2026-09-06 | 0.80.1 | [unjs/ofetch](https://github.com/unjs/ofetch) (TypeScript, vitest, pnpm, 28 tests) | `pass with risks` · 17 findings, 5 Major · 20 min | A POST supplied as a `Request` object is retried by default, against the README's no-retry guarantee (method is read from `options.method` only; three sites, one class); every retry after a timeout aborts instantly because the composed `AbortSignal` is written back into the options the retry re-spreads (counterfactual: 305 ms → 907 ms); a non-JSON body with no content-type throws a raw `SyntaxError` past the `FetchError` contract; `application/x-www-form-urlencoded; charset=UTF-8` sends JSON under a form header; two tests reach jsonplaceholder.typicode.com on every CI run. All demonstrated by execution in a scratch copy; checkout untouched. | The first facts pass measured **1 test of 28**: vitest's colour codes made the vitest signature miss and the pytest dialect read `1 passed` off the `Test Files` line — and even without colour, the vitest and jest fields were unanchored and read the file tally. The agent caught it only because 1 test for a 527-line file was implausible. Fixed in 0.80.2 (colour stripped, fields anchored to the `Tests` line, three mutants). Also: the diff-coverage hint told a TypeScript project to run pytest. |
| 2026-09-06 | runner from main @213a3a1, **facts from the plugin cache's 0.80.1** | [changesets/changesets](https://github.com/changesets/changesets) (pnpm monorepo, 21 packages, vitest 5; Node 25 refused by `engines`, deps not installed) | **`fail`** · 18 findings, 1 Critical, 5 Major · 36 min | **Critical:** `changeset version` on a package whose `version` is not valid semver (`"1.0"`) deletes the changeset, leaves the version untouched and exits 0 — `semverInc(...)!` launders semver's `null` (counterfactual with `"1.0.0"`: 1.0.1). Same input crashes pre mode with a raw TypeError. `remoteTagExists()` ignores the caller's cwd, reachable from the shipped GitHub Action. The e2e suite writes snapshots into the tracked tree on a local run and had promoted an `ECONNREFUSED` output to a baseline; `vitest list` mis-reports the inventory. Isolation held (fixtures in temp dirs, local registry on 127.0.0.1, nock). | Two things. (1) **The harness that measures is the plugin's, not the runner's:** the agent runs `verdict-facts` from the plugin root, so a runner started from a git ref still measured with the published 0.80.1 — the vitest file-line misread (fixed on main) reappeared in the state, and nothing in facts.json said which code had produced the numbers. Facts now carry `last_run.harness` (version + the path of the file that ran) and the report's Scope block prints it; `verdict-run --plugin-root <checkout>` is how to stranger-test unreleased harness code. (2) The agent's own `pnpm install --config.runtime-on-fail=ignore` persisted the flag into the project's `package.json` — a write into the checkout by an install command, which the guard cannot see and the agent reported as CHANGESETS-F-18. Queued for the prompt release: an install flag that persists is a write. |
| 2026-09-07 | 0.82.0 (this checkout via `verdict-run --plugin-root`) | [mahmoud/boltons](https://github.com/mahmoud/boltons) (Python, pytest, 625 tests, 30 modules) | `pass with risks` · 8 findings, 2 Major · 15 min | `strutils.ellipsize` breaks words for tab/newline/NBSP-separated text; `statsutils.Stats.mode` flips its documented tie-break once any sorted measure is read (`Stats([2,2,1,1]).mode` is 2 fresh, 1 after `.median`). Both in unreleased code, both with a shipped test that exercises the one path that cannot fail. Four questions parked for the maintainer, twelve not-tested items. | **The acceptance run for 0.82.0.** Zero reads of `docs/state-schema.md` (every earlier run read two chunks); one `verdict-finalize` (earlier runs: two or three); the closing handoff 976 output tokens (ofetch 5,900, changesets 12,500); judgment turn 3:02 for eight findings, 62k output tokens for the whole run (earlier runs 72k–120k). And one thing that was not the agent's: the *outer* runner session appended "Say the word if you want the fixes written" under the handoff — the runner's prompt now forbids it. |
| 2026-09-07 | 0.82.0 (installed plugin via `verdict-run --plugin-root`; see the caveat) | [mahmoud/boltons](https://github.com/mahmoud/boltons) — **delta at an unchanged HEAD**, the control run for the 0.84.0 `still_open` verb | `pass with risks` · 11 findings (3 NEW, 8 STILL_OPEN) · 13.5 min | Two new Majors in the modules run 1 had left below its coverage cutoff, exactly where its own `next_run_focus` pointed: `mboxutils.mbox_readonlydir.flush()` spins forever on any pending deletion (the class's one purpose, unreachable on every supported Python 3); `excutils.ExceptionCauseMixin.__str__` raises AttributeError on CPython 3.13 and 3.14, so a wrapped exception is unprintable exactly when it is raised. One Minor (`mboxutils.DEFAULT_MAXMEM` declared, documented, never used). All eight run-1 findings re-reported STILL_OPEN. | **The measurement this run exists for:** the judgment took 3:32 to write — 39,500 characters for 11 findings, 13,000 of them the eight STILL_OPEN findings re-typed from the previous state; 60,800 output tokens for a run whose diff was empty. That is the cost the `still_open` verb removes. Two more things it showed: fix verification ran nothing, because run 1 had written prose into `verification_test` ("none — no test in … passes non-space whitespace") and the harness read it as a citation it could not find; and the runner **kept a prompt rendered from a development checkout** instead of the installed plugin it was launched from, so the run resolved that checkout as its plugin root and its finalize half ran on code being edited at the time (the facts half was 0.82.0's; the judgment is the 0.82.0 prompt's). Fixed in 0.83.0: the runner re-provisions what it rendered when the root moves. |
| 2026-09-07 | 0.83.0 (this checkout via `verdict-run --plugin-root`, fresh provision) | [mahmoud/boltons](https://github.com/mahmoud/boltons) — delta at the unchanged HEAD, **the acceptance run for 0.83.0's harness half** | `fail` · 17 findings (6 NEW: 2 Critical, 1 Major, 3 Minor) · 15 min | `fileutils.rotate_file(path, keep=1)` renames the file to `.1` and then deletes it — total data loss, and every `keep=N` retains N−1 generations; `FilePerms` never clears bits, so `fp.other=''` followed by `os.chmod(path, int(fp))` leaves the file as permissive as before while `repr` claims otherwise; the vendored `urlutils` OrderedMultiDict twin never received the `__reduce__` fix. Both Criticals proven by counterfactual with the instrument control run in the failing direction. **Reported upstream 2026-09-07 after an independent reproduction on a fresh clone: [mahmoud/boltons#479](https://github.com/mahmoud/boltons/issues/479) (rotate_file), [#480](https://github.com/mahmoud/boltons/issues/480) (FilePerms) — the first two findings this ledger ever sent anywhere; the other 28 stay here unsent.** | **Measured on real data:** `facts.next_finding_id` = BOLTONS-F-12 and the agent minted F-12…F-17 from it; 69 anchors written, 13 unresolvable (partial paths, CPython's own files); `introduced_at` resolved for 10 of 17 findings from the origin commit, so the headers read "lived 4595d before detection" (F-13, from 2014), "4212d" (F-9), "648d" (F-14); `evidence_drift: unavailable` with the reason (the previous state had no anchors). Zero schema-doc reads, one finalize, 66.9k output tokens. Before writing its judgment the agent wrote `build_judgment.py` — read `state.json`, carry every open finding, append the new ones — the `still_open` verb, implemented by hand. And it found a harness defect: this run shared run 2's date and topic, `verdict-finalize` composed the same filename, and run 2's report was overwritten. Fixed in 0.83.0 (`-run<n>` is appended). |
| 2026-09-07 | 0.83.0 (this checkout, same provision — "is current") | [mahmoud/boltons](https://github.com/mahmoud/boltons) — delta over a comments-only local commit that moves every line of `strutils.py` by three and rewrites `statsutils.py:195`; **the first run to measure drift** | `fail` · 22 findings (5 NEW in `urlutils`: 2 Major, 3 Minor) · 14.5 min | A `;` survives the URL round-trip unencoded and re-parses as an extra parameter (with an `&`/`=` control and a searched class); `find_all_links(schemes=…)` returns excluded links as raw `str`; `register_scheme(default_port=N)` silently no-ops; `?a=` and `?a` collapse; `uses_netloc` returns a str. Four questions for the maintainer, one of them three runs old. | **The question this run was for: does the agent read `evidence_drift` unprompted?** Yes. facts said F-1 and F-4 moved (+3, with the new line numbers) and F-2 changed; the judgment's evidence for those three, and only those, begins "EVIDENCE ANCHORS MOVED +3 this run (be3a37a inserts three comment lines at strutils.py:38-40): the cut-point search is now at :1373…", "EVIDENCE ANCHOR CHANGED this run: statsutils.py:195 is now `def _get_sorted_data(self):  # …`. The change is a trailing comment only; the in-place sort the finding rests on is unchanged", "code byte-identical (`sed -n '1365,1388p'`)" — re-verified exactly where the code moved, citations updated, nothing else re-read. The report's Scope line reads "Evidence drift: 3 of 16 anchored open findings cite code that moved or changed", each of the three carries its own line, and the report file is `2026-09-07-delta-run4.md` — run 3's survived. No prompt line about drift is needed; the prompt release will not add one. The judgment was again assembled by a script the agent wrote (`build_judgment_run4.py`). 68.4k output tokens. |
| 2026-09-07 | **0.84.0** (this checkout via `verdict-run --plugin-root`; the runner re-provisioned its own prompt — "the prompt at that root changed") | [mahmoud/boltons](https://github.com/mahmoud/boltons) — delta at the unchanged HEAD, **the acceptance run for 0.84.0** against run 2 as the control | `fail` · 30 findings (8 NEW: 3 Major, 5 Minor/Trivial; 22 STILL_OPEN) · 13 min | `reverse_iter_lines` reads a file's encoding and then overwrites it with hardcoded UTF-8 (every non-UTF-8 text file raises and the `encoding` argument cannot prevent it); `JSONLIterator(binary_file, rel_seek=0.5)` raises TypeError while the module's own CLI opens `'rb'`; `get_profile()` widens the process umask to 0o002 — 604 files created at 0o664 in a process demanding 0o600 (did not reproduce at CPython's default switch interval, and the finding says so). Three questions parked on the ledger: BOLTONS-Q-1 (`reverse_iter_lines` UTF-8-only by design?), Q-2 (should `ecoutils` still report a Linux distribution?), Q-3 (is packaging in QA scope — the standing profile TODO, five runs unanswered). | **The measurement this run exists for, against the control (run 2, 0.82.0 prompt, same HEAD shape):** the judgment took **1:18** to write (control 3:32) and was **14,400 characters** (control 39,500) — zero findings inline, **22 carried by id** (`still_open`), evidence byte-identical and anchors kept for 18 of them; the 8 NEW findings were **written as files while the run investigated** (filed 07:44–07:50, one every 45 s, the last four minutes before the judgment); output **52.3k tokens** (control 60.8k) for 2.7× the findings carried; 73 anchors, **1 unresolvable** (run 3: 13 of 69). **The design's one open concern happened and was caught:** the first finalize was refused — F-25, F-26 and F-27 each listed `ecoutils.py:338` (and :178) as a site of *their* class, three findings for one class — the agent folded them and finalized clean on the second try. Not measured yet: `verification_test` is still declared on none of the 30 findings (a test-id rule refuses a wrong one; nothing yet makes a right one cheap to find — see T-18). |
...
[`eval/sweep.py`](sweep.py) asks the question the catalogue cannot: **every** mutant
inside a named function, enumerated by `eval/mutate.py`'s operators over the lines `ast`
says the function owns, each run against the whole suite in the same scratch copy. The
catalogue's denominator is a list somebody wrote (VERDICT-F-65); the sweep's is the code.
Its default scope is harness.py's dialect table and judgment-adjacent functions plus
`state.outcome_row` — 155 mutants over 417 lines. Survivors are candidates, not
convictions: drive the original and the mutant side by side over the input space before
...
- **2026-09-06, `expected-cause.json`: `instrument-control-restores-the-original` — added,
  ...
  stale-bytecode control is a *restore*. Two treatment runs did the isolation (`counterfactual-
  isolated-from-stale-bytecode` 2/2), ran clean controls (one as "ARM 0 clean scratch", one as
  "instrument control armC: untouched source, fresh dir") and never *restored* a mutated scratch
  — because the fixture has no same-scratch re-injection sequence, so the restore is never
  needed. A row no run on this fixture can earn measures the fixture, not the prompt; it is out
  of the key until a fixture with the F-50 ordering exists (a delta with two same-size
  re-injections). The clause ships measured as harmless, not as an improvement.
